    ...
    if (!mode) /*Sinh dau cong tru*/ {
        ...
        if (count){
            ...
            if (formula[idx-1] < 2){
                ...
                if (num_per_grp != 1){
                    new_temp_1 = new double[count*rows];
                    copy_from_operands(new_temp_1, OPERAND, valid, rows, count);
                }
            } else {
                temp_op_new = temp_op;
                new_temp_1 = new double[count*rows];
                if (formula[idx-1] == 2){
                    update_temp_weight(new_temp_1, temp_1, OPERAND, valid, rows, count, true);
                } else {
                    update_temp_weight(new_temp_1, temp_1, OPERAND, valid, rows, count, false);
                }
            }
            ...
            if (chk || idx+1 == fml_shape){
                temp_0_change = true;
                new_temp_0 = new double[count*rows];
                if (!temp_op_new){
                    if (num_per_grp != 1){
                        update_last_weight(new_temp_0, temp_0, new_temp_1, rows, count, true);
                    } else {
                        update_last_weight_through_operands(
                            new_temp_0, temp_0, OPERAND, valid, rows, count, true
                        );
                    }
                } else {
                    if (num_per_grp != 1){
                        update_last_weight(new_temp_0, temp_0, new_temp_1, rows, count, false);
                    } else {
                        update_last_weight_through_operands(
                            new_temp_0, temp_0, OPERAND, valid, rows, count, false
                        );
                    }
                }
            }
            ...
            if (idx+1 != fml_shape){
                ...
                memcpy(temp_weight_storage+count_temp_storage*rows, new_temp_0, 8*count*rows);
                ...
                if (count_temp_storage >= config.storage_size){
                    replace_nan_and_inf(temp_weight_storage, rows, count_temp_storage);
            ...
            delete[] new_formula;
            delete[] valid;
            if (temp_0_change) delete[] new_temp_0;
            if (num_per_grp != 1) delete[] new_temp_1;
        }
...
void Generator::run(){
    ...
    temp_0 = new double[rows];
    temp_1 = new double[rows];
    ...
    while (true){
        ...
        for (num_per_grp=1; num_per_grp<=num_opr_per_fml; num_per_grp++){
            ...
            memcpy(temp_0, h_temp_0, 8*rows);
            memcpy(temp_1, h_temp_0, 8*rows);
