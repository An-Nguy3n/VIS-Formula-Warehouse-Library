#pragma once
#include "NoBalanceValueKernel.cu"
...
: Generator(config_path) {
    cudaMalloc((void**)&d_profits, 8*rows*(config.storage_size+cols));
    cudaMalloc((void**)&d_profits_origin, 8*rows*(config.storage_size+cols));
    cudaMalloc((void**)&d_results, 8*config.num_cycle*(config.storage_size+cols));
    h_results = new double[config.num_cycle*(config.storage_size+cols)];

    cudaMalloc((void**)&d_temp_wgt, 8*rows*(config.storage_size+cols));
    cudaMalloc((void**)&d_temp_prf, 8*rows*(config.storage_size+cols));

    start = chrono::high_resolution_clock::now();

    for (int i=0; i<config.storage_size+cols; i++)
        cudaMemcpy(d_profits_origin+i*rows, PROFIT, 8*rows, cudaMemcpyHostToDevice);
}


NoBalanceValueFilter::~NoBalanceValueFilter(){
    cudaFree(d_profits);
    cudaFree(d_profits_origin);
    cudaFree(d_results);
    delete[] h_results;
    cudaFree(d_temp_wgt);
    cudaFree(d_temp_prf);
...
bool NoBalanceValueFilter::compute_result(bool force_save){
    cudaMemcpy(d_profits, d_profits_origin, 8*rows*count_temp_storage, cudaMemcpyDeviceToDevice);

    get_noBalance_value<<<count_temp_storage/256 + 1, 256>>>(
        temp_weight_storage,
        d_profits,
        d_temp_wgt,
        d_temp_prf,
    ...
    //
    cudaMemcpy(h_results, d_results, 8*count_temp_storage*config.num_cycle, cudaMemcpyDeviceToHost);
